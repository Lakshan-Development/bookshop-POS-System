...
$dlist = "";
$tot = 0;
$sql = "SELECT * FROM sales WHERE date = '{$date}' and type = 1";
	$result = $conn->query($sql);
	if ($result->num_rows > 0){
		while($row = $result->fetch_assoc()){
			$id = $row['id'];
            ...
            $dis = $sprice * ($sumdiscount/100);
            $sum = ($sprice * $qty) - ($sprice * ($sumdiscount/100));
            $tot += $sum;
            $sums = $sum;
            $dlist .= "<tr>
                <td>{$mid}</td>
        ...
        //$dlist .= "<tr><th colspan = '4'>Total</th><th>Rs. {$tot}</th></tr>";
    }
    else{
        $dlist = "";
    }
	...
	if ($resultunpaid->num_rows > 0){
        ...
        $dlist .= "<tr><th colspan = '5' style = 'font-weight:bold;color:black;'>Total</th><th style = 'font-weight:bold;color:black;'>Rs. {$tot}</th></tr>";
    }
?>
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h2 mb-0" style = "color:#000;font-family: 'Impact', sans-serif;letter-spacing: 1px;">Daily Sales</h1>
                    </div>
    ...
    <script>
        window.onload = function () {
            ...
            .addEventListener("click", () => {
                ...
                var opt = {
                    margin: 0.2,
                    filename: '<?php echo $date;?> Daily Sales.pdf',
