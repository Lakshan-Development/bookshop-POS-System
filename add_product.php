...
if(!isset($_SESSION['remadminlogin'])){
	...
	if ($result->num_rows > 0){
		...
		while($row = $result->fetch_assoc()){
			...
			$bprice = $row['bprice'];
			$bdiscount = $row['bdiscount'];
			$sprice = $row['sprice'];
			$sdiscount = $row['sdiscount'];
			$qty = $row['qty'];
		
            $dlist .= "<tr><td>{$itemcode}</td>
            <td>{$itemname}</td>
            <td>{$suplier}</td>
            <td>Rs. {$bprice}</td>
            <td>{$bdiscount}%</td>
            ...
            if($type == 1){
                $dlist .= "<td>
                <a href = 'edititems?delitem={$idu}'><i class=\"fas fa-fw fa-pen\"></i></a> | 
                <a href = 'processing?delitem={$idu}'><i class=\"fas fa-fw fa-trash\"></i></a>";
            }
            $dlist .= "</tr>";
		}
	}
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <h1 class="h2 mb-0" style = "color:#000;font-family: 'Impact', sans-serif;letter-spacing: 1px;">
                            ...
                            <?php
                            if($type == 1){
                            echo "<a href=\"addnewproduct\" class = \"btn btn-primary ml-4\" style = \"font-family: sans-serif;\">Add New Item</a>";
                            }
                            ?>
                        </h1>
                    </div>
